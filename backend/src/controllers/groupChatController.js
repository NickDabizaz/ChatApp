...
const GroupChatController = {
    createGroupChat: async (req, res) => {
      ...
      addMemberToGroup: async (req, res) => {
        try {
          ...
          if (groupChat.admin.toString() !== memberId.toString() && !groupChat.members.includes(memberId)) {
  ...
  getAllMessagesInGroup: async (req, res) => {
    try {
      const groupId = req.params.groupId;
      const groupChat = await GroupChat.findById(groupId).populate('messages.senderId', 'name _id');
      if (!groupChat) {
        return res.status(404).json({ error: 'GroupChat not found' });
      }
  ...
  getGroupDetails: async (req, res) => {
    try {
      const groupId = req.params.groupId;
      const groupDetails = await GroupChat.findById(groupId).populate('admin members', 'name _id');
      if (!groupDetails) {
        return res.status(404).json({ error: 'GroupChat not found' });
      }
